...
public class SongRequest extends Request {
	...
	public SongRequest(DaapHost h, Song s, long bytes) throws PasswordFailedException, BadResponseCodeException, IOException {
	    super(h);
	...
	protected void addRequestProperties() {
	    httpc.addRequestProperty("Host", "" + host.getAddress() + ":" + host.getPort() + "/");
		httpc.addRequestProperty("Accept", "*/*");
		httpc.addRequestProperty("Cache-Control", "no-cache");
	    super.addRequestProperties();
		httpc.addRequestProperty("Client-DAAP-Request-ID", ""+host.getThisRequestNumber());
		if (skip_bytes > 0)
	...
	protected String getRequestString() {
	    ...
	    ret += "/items/" + song.getId() + "." + song.getFormat();
	...
	protected void readResponse() throws IOException {
	    ...
	    if (len == -1)
	        return;
		b = new BufferedInputStream(httpc.getInputStream());
	}
	...
	/**
	 *  Gets the InputStream for the requested song
	 *
